...
namespace UnityEngine.UI
{
    ...
    public abstract class LoopScrollRect : UIBehaviour, IInitializePotentialDragHandler, IBeginDragHandler, IEndDragHandler, IDragHandler, IScrollHandler, ICanvasElement, ILayoutElement, ILayoutGroup
    {
        ...
        [SerializeField]
        private RectTransform m_ItemMap;
        public RectTransform ItemMap
        {
            get
            {
                if(m_ItemMap == null)
                {
                    m_ItemMap = new GameObject("ItemMap").AddComponent<RectTransform>();
                    m_ItemMap.position = new Vector3(10000,10000,10000);
                    m_ItemMap.SetParent(content.parent,false);
                }
                return m_ItemMap;
        ...
        public Dictionary<int, RectTransform> m_updatelist = new Dictionary<int, RectTransform>();
        ...
        //==========LoopScrollRect==========
        private void ReturnObjectAndSendMessage(Transform go,int idx)
        {
            //go.SendMessage("ScrollCellReturn", SendMessageOptions.DontRequireReceiver);
            //SG.ResourceManagerExt.Instance.ReturnObjectToPool(go.gameObject);
            go.SetParent(ItemMap,false);
            //go.gameObject.SetActive(false);
            dataSource.ProvideData(go,idx,false);
        ...
        public void ClearCells()
        {
            if (Application.isPlaying)
            {
                ...
                //totalCount = 0;
                //objectsToFill = null;
                for (int i = content.childCount - 1; i >= 0; i--,itemTypeEnd--)
                {

                    ReturnObjectAndSendMessage(content.GetChild(i),itemTypeEnd);
                }
                itemTypeStart = 0;
                itemTypeEnd = 0;
                totalCount = 0;
        ...
        public void SetLuaFunction(LuaInterface.LuaFunction lf)
        {
            if (lf != null)
            {
                dataSource.cb = lf;
        ...
        public void RefreshCells()
        {
            if (Application.isPlaying && this.isActiveAndEnabled)
            {
                ...
                var tempindex = itemTypeStart;
                // recycle items if we can
                for (int i = 0; i < content.childCount; i++)
                {
                    if (itemTypeEnd < totalCount)
                    {
                        dataSource.ProvideData(content.GetChild(i), itemTypeEnd,true);
                        itemTypeEnd++;
                    }
                    else
                    {
                        ReturnObjectAndSendMessage(content.GetChild(i), tempindex);
                        i--;
                    }
        ...
        public void RefillCellsFromEnd(int offset = 0)
        {
            ...
            var tempindex = itemTypeEnd;

            for (int i = m_Content.childCount - 1; i >= 0; i--,tempindex--)
            {
                ReturnObjectAndSendMessage(m_Content.GetChild(i),tempindex);
            }
        ...
        public void RefillCells(int offset = 0)
        {
            ...
            {
                ReturnObjectAndSendMessage(m_Content.GetChild(i), tempindex+i);
            }
        ...
        protected float DeleteItemAtStart()
        {
            ...
            for (int i = 0; i < contentConstraintCount; i++)
            {
                ...
                if (reverseDirection == false)
                {
                    ...
                    ReturnObjectAndSendMessage(oldItem, itemTypeStart);
                    itemTypeStart++;
                }
                else
                {
        ...
        protected float DeleteItemAtEnd()
        {
            ...
            for (int i = 0; i < contentConstraintCount; i++)
            {
                ...
                {
                    itemTypeEnd--;
                    ReturnObjectAndSendMessage(oldItem, itemTypeEnd);
                }
                else
                {
        ...
        private RectTransform InstantiateNextItem(int itemIdx)
        {
            ...
            var childCount = ItemMap.childCount;
            RectTransform itemobj = null;
            if (childCount == 0)
            {
            ...
            {
                itemobj = ItemMap.GetChild(0).GetComponent<RectTransform>();
            }
            RectTransform nextItem = itemobj.GetComponent<RectTransform>();
            nextItem.transform.SetParent(content, false);
            nextItem.gameObject.SetActive(true);
            m_updatelist[itemIdx] = nextItem;
        ...
        protected virtual void LateUpdate()
        {
            ...
            var iter = m_updatelist.GetEnumerator();
            while (iter.MoveNext())
            {
                dataSource.ProvideData(iter.Current.Value, iter.Current.Key,true);
            }
            m_updatelist.Clear();
